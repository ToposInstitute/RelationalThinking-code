...
loopedBulb_blueprint = WiringDiagram([], [:Bulb1_state, :Bulb2_state])
...
add_wires!(loopedBulb_blueprint, Pair[
    ...
    (Box2, 1)    => (Box1, 1), # output of Box2 connected to 1st input Box 3
])
...
Bulb1 = Bulb2 = DiscreteMachine{Bool}(1,1,1, Transition, Readout)

# Compose
Looped_bulbs = oapply(loopedBulb_blueprint, [Bulb1, Bulb2]) 
...
map(sol) do u
    return (Bulb_1= getState(u[1]), Bulb_2=getState(u[2]))
end |> pretty_table
...
result =  map(sol) do u
    return (getStateColor(u[1]), getStateColor(u[2]))
end
